...
public class ZombieSkillManager {
    ...
    private static final String SKILL_TAG =
            "SmartZombieSkill";
    ...
    private static void assignRandomSkill(
        ...
        int normal =
                ZombieConfig.ZOMBIE_NORMAL_CHANCE.get();

        int dodge =
                ZombieConfig.ZOMBIE_DODGE_CHANCE.get();

        int shield =
                ZombieConfig.ZOMBIE_SHIELD_CHANCE.get();

        int flint =
                ZombieConfig.ZOMBIE_FLINT_CHANCE.get();

        int breakBlock =
                ZombieConfig.ZOMBIE_BREAK_BLOCK_CHANCE.get();
        ...
        if (total <= 0) {
            ...
            zombie.getPersistentData().putString(
                    SKILL_TAG,
                    ZombieSkill.NONE.name()
            );
    ...
    public static boolean hasSkill(
        ...
        return zombie.getPersistentData()
                .contains(SKILL_TAG);
    }

    public static ZombieSkill getSkill(
            Zombie zombie) {
